...
const envbeeInit = function envbeeInit(parameters) {
  ...
  let cacheDir = path.join(appPaths.cache, `envbee_${safeCacheKey}`);
  // Prefer OS-specific persistent cache directories; if unavailable in restricted runtimes,
  // gracefully fallback to temp storage. ROADMAP: introduce a pluggable cache interface so users can provide custom backends.
  try {
    fs.mkdirSync(cacheDir, { recursive: true });
  } catch (err) {
    logger.warn("Falling back to temp cache directory due to path error", err);
    cacheDir = path.join(os.tmpdir(), "envbee", `envbee_${safeCacheKey}`);
    fs.mkdirSync(cacheDir, { recursive: true });
  }
