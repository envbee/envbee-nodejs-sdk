...
const envbeeInit = function envbeeInit(parameters) {
  ...
  const { key, secret, encKey } = parameters;
  if (!key || !secret) {
  ...
  const tryDecryptValue = async function tryDecryptValue(value) {
    ...
    if (!aesgcm) {
      throw new Error("Received encrypted value, but no encryption key was provided");
    }
    ...
    try {
      ...
      const ciphertext_and_tag = decoded.subarray(12); // ciphertext + tag

      const decipher = crypto.createDecipheriv("aes-256-gcm", aesgcm, iv);
      const authTag = ciphertext_and_tag.subarray(ciphertext_and_tag.length - 16);
      const ciphertext = ciphertext_and_tag.subarray(0, ciphertext_and_tag.length - 16);
  ...
  const doFetch = async function doFetch(urlBase, urlPath, params = {}) {
    try {
      ...
      if (res.status === 200) {
        ...
        logger.error(`Request failed. Status code: ${res.status}. Error: ${JSON.stringify(errorBody)}`);
      ...
      logger.fatal(`Error during the request to ${urlBase}${urlPath}: ${error?.message || error}`, error);
  ...
  const getVariableValueContentByName = async function getVariableValueContentByName(variableName) {
    ...
    try {
      const result = await doFetch(apiURL, `${VARIABLES_VALUES_BY_NAME_V1}/${variableName}/content`);
      try {
      ...
      const cached = await storage.defaultInstance.get(variableName);
      if (!cached) {
